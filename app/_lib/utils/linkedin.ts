...
export const getMultipleFormattedJob = (JobList: Jobs[]) => {
  const topContentIndex = Math.floor(
    Math.random() * linkedinJobTemplatesTop.length
  );
  const topContent = linkedinJobTemplatesTop[topContentIndex];
  const bottomContentIndex = Math.floor(
    Math.random() * linkedinJobTemplatesBottom.length
  );
  ...
  const formattedJob =
    topContent + middleContent + bottomContent + " " + getRandomHashtags(5);
  return formattedJob;
};
