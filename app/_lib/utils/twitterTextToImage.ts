...
export const generateTwitterLikeImage = async (content: string): Promise<string> => {
    return new Promise((resolve, reject) => {
        const canvas = createCanvas(800, 500)
        const ctx = canvas.getContext("2d")
        if (!ctx) return reject("Canvas context not found.")
        const backgroundImageSrc = path.join(process.cwd(), "public", "post-background.jpg")
        const logoImageSrc = path.join(process.cwd(), "public", "onedotjob-logo.png")
        ...
        loadImage(backgroundImageSrc)
            .then((backgroundImage) => {
                ...
                ctx.quadraticCurveTo(x + tweetBoxWidth, y, x + tweetBoxWidth, y + borderRadius)
                ...
                ctx.quadraticCurveTo(x, y + tweetBoxHeight, x, y + tweetBoxHeight - borderRadius)
                ...
                loadImage(logoImageSrc)
                    .then((logoImage) => {
                        ...
                        drawMultilineText(ctx, content, x + 50, y + 140, lineHeight, maxTextWidth)
                        // Add hashtags
                        ...
                        ctx.fillText(getCurrentFormatTimeStamp(new Date()), x + 40, y + 370)
