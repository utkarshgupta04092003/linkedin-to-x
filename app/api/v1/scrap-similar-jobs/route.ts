import { extractSimilarJobsData, fetchData } from "@/app/_lib/data-fetchers/extractData"
import { updateDB } from "@/app/_lib/data-fetchers/globals"
import { filterData, formatResponseMessage, prisma } from "@/app/_lib/utils/globals"
...
export async function GET() {
    try {
        ...
        return NextResponse.json(
            formatResponseMessage(scrapedJobs?.updatedRows?.length, endTime - startTime, {
                updatedJobs: scrapedJobs?.updatedJobs,
            }),
            { status: 200 }
        )
    } catch (error) {
...
const scrapSimilarJobs = async () => {
    ...
    return { updatedRows: scrapedData, updatedJobs: successJobs }
}
